class EventBooker {
    ...
    async loadBookings() {
        ...
        try {
            // В реальном приложении здесь будет запрос к API бронирований
            // Для демо создадим моковые данные
    ...
    generateMockBookings() {
        ...
        this.events.forEach(event => {
            for (let i = 0; i < Math.min(3, event.TotalSeats - event.Available); i++) {
                mockBookings.push({
                    id: `booking_${event.ID}_${i}`,
                    event_id: event.ID,
                    ...
                    expires_at: new Date(Date.now() + (30 - i) * 60000).toISOString(),
                    confirmed_at: i % 2 === 0 ? new Date().toISOString() : null
                });
            }
        });
